...
export function QueryControl( props ) {
	const { value, onApplyQuery } = props;

	const [ innerQuery, setInnerQuery ] = useState( undefined );
	const query = queryString.stringify( value, { encode: false } );
	...
	const noticeContent = sprintf(
		...
		__(
			'It is possible to alter some properties of the WordPress query for fetching cards. The allowed params of WP-API can be found %1$shere%2$s.',
			'statik'
		),
		'<a target="_blank" rel="noopener noreferrer" href="https://developer.wordpress.org/rest-api/reference/posts/#arguments">',
		'</a>'
	);
